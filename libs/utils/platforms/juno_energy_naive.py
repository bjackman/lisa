...
a53s = [0, 3, 4, 5]
a53_pd = PowerDomain(cpus=a53s, idle_states=[], parent=None)

def a53_cpu_node(cpu):
    cpu_pd=PowerDomain(cpus=[cpu],
                       parent=a53_pd,
                       idle_states=["WFI", "cpu-sleep-0", "cluster-sleep-0"])
...
a57s = [1, 2]
a57_pd = PowerDomain(cpus=a57s, idle_states=[], parent=None)
...
juno_energy_naive = EnergyModel(levels=juno_energy_levels)
