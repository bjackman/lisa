...
################################################################################
    ...
    def _setup_cpufreq(self, tc):
        ...
        if 'freqs' in cpufreq:
            ...
            logging.info(r'%14s - CPU frequencies: %s',
...
################################################################################
    ...
    def _freeze_userspace(self):
        if 'cgroups' not in self.target.modules:
            raise RuntimeError(
                "freeze_userspace: devlib cgroups module not loaded. "
                "Ensure it's listed in your target/test modules")
        controllers = [s.name for s in self.target.cgroups.list_subsystems()]
        if 'freezer' not in controllers:
            self._log.warning('freeze_userspace: no freezer cgroup controller')
            return False

        exclude = self.critical_tasks[self.te.target.os]
        self._log.info('Freezing all tasks except %s', exclude)
